...
namespace Melchior.Request.Common
{
    ...
    public class VKRequestCustomException : Exception
    {
        ...
        public VKRequestCustomException(VKData dataErrorCode, VKData dataErrorMessage)
            : base("VKRequestCommand Exception: " + dataErrorCode.GetTextContent() + "\n" + dataErrorMessage.GetTextContent())
        {
            Code = Int32.Parse(dataErrorCode.GetTextContent());
        }
        public VKRequestCustomException(VKData data)
            : this(data.GetField("error_code"), data.GetField("error_msg"))
        {
        }
    }
}